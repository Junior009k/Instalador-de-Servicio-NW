from Middleware.Validador import validateService
import win32com.client
import pyodbc

def manageUDL(server,database,path,loggin,datetime):
    ...
    if(validateService(path,"STE","Middleware","ConfigFiles")):
        ...
        #e-Flow
        createsUDL("SOF","SOF","SOF",server,database,path,loggin,datetime)
        createsUDL("STE","usr_STE","usr_STE",server,database,path,loggin,datetime)
        createsUDL("STE_HD","usr_STE","usr_STE",server,database,path,loggin,datetime)
        createsUDL("STE_ServerGroup","usr_STE","usr_STE",server,database,path,loggin,datetime)
        ...
        #Citas
        createsUDL("SOF","SOF","SOF",server,database,path,loggin,datetime)
        createsUDL("SSP","usr_SSP","usr_SSP",server,database,path,loggin,datetime)
        ...
        createsUDL("SOF","SOF","SOF",server,database,path,loggin,datetime)
        createsUDL("OpinionPoll","usr_STE","usr_STE",server,database,path,loggin,datetime) 
    else:
        loggin.error(f' {datetime.datetime.now() }: No se encontraron los udls')
        print("No se encontraron los udls")

def createsUDL(name,user,password,server,database,path,loggin,datetime):
    try:
        print(f"{path} {name}")
    ...
    # print(contentS.split(';',4))
        ...
        loggin.error(f' {datetime.datetime.now() }: No se ha identificado correctamente el udl, por favor revisarlos')
...
def validateConnection(server, database,loggin,datetime):
    try:
        conn_str = f'DRIVER={{SQL Server}};SERVER={server};DATABASE={database};UID=SOF;PWD=SOF'
        ...
        print("Error: No se pudo conectar al servidor. Verifica la dirección del servidor y la red.")
        loggin.error(f' {datetime.datetime.now() }:Error: No se pudo conectar al servidor. Verifica la dirección del servidor y la red.')
    except pyodbc.OperationalError as e:
        print(f"Error de conexión: {e}. Verifica las credenciales y que el servidor esté en ejecución.")
        loggin.error(f' {datetime.datetime.now() }: Error de conexión: {e}. Verifica las credenciales y que el servidor esté en ejecución.')
    except Exception as e:
        print(f"Error inesperado: {e}. Verifica la configuración de la base de datos y la red.")
        loggin.error(f' {datetime.datetime.now() }: Error inesperado: {e}. Verifica la configuración de la base de datos y la red.')
